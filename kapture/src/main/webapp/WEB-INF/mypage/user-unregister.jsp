...
<div id="app" class="flex max-w-6xl mx-auto px-6 py-8 gap-10">
    ...
    <div class="flex-1">
        <div v-if="!unregisterFlg" class="flex flex-col items-center justify-center mt-40 text-center gap-6">
            ...
            <div class="space-x-4">
                <button @click="unregisterY" class="btn-unregister bg-blue-950 text-white px-4 py-2 rounded hover:bg-blue-700">예</button>
                <button @click="unregisterN" class="btn-unregister bg-gray-300 px-4 py-2 rounded hover:bg-gray-400">아니오</button>
            </div>
        </div>
        ...
        <template v-if="unregisterFlg && userInfo.password.length > 20">
            <div class="flex items-center justify-center mt-40 gap-4">
                ...
                <input type="password" v-model="confirmPassword" placeholder="비밀번호를 입력해주세요." @keyup.enter="fnUserUnregister" class="border px-3 py-2 rounded w-64" />
                <button @click="fnUserUnregister" class="bg-blue-950 text-white px-4 py-2 rounded hover:bg-blue-700">확인</button>
            </div>
        </template>
    </div>
</div>
